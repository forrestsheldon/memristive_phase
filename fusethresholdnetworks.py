...
class FuseThresholdNetwork(rnets.ResistorNetwork):
    ...
    def adjust_current(self, current):
        """
        Adjusts the applied voltage and voltages throughout the network to have a given current.
        """
        ext_currents = self.external_current()
        total_current = ext_currents[ext_currents>0].sum()
        self.external_voltages = self.external_voltages * current / total_current
        self.voltages = self.voltages * current / total_current
        
    def switching_step_cc(self):
        counter = []
        self.solve_voltages('spsolve')
        self.calculate_currents()
        ext_currents = self.external_current()
        current = ext_currents[ext_currents>0].sum()
        num_switched = self.switch_memristors()
        while num_switched != 0:
            counter.append(num_switched)
            self.solve_voltages('spsolve')
            self.adjust_current(current)
            self.calculate_currents()
            num_switched = self.switch_memristors()
        return counter
